...
def call(duration = 1, unit = "HOURS", Closure body) {
  ...
  node('linux') {
    duration  = duration  ?: 1
    unit      = unit      ?: 'HOURS'
    assert (duration instanceof Integer)  : "WorkflowLibs :: plLinux :: duration must be a Integer"
    assert (unit instanceof String)       : "WorkflowLibs :: plLinux :: unit must be a String"
